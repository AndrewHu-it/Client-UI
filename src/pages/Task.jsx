...
export default function Task() {
  ...
  const updateXResolution = e => {
    ...
    if (autoAdjustPixelRatio) {
      ...
      setNumTasks(prev => Math.min(prev, Math.min(250, Math.floor(clampedX/2), Math.floor(newY/2))));
    } else {
      setResolution({ ...resolution, x_resolution: clampedX });
      // clamp tasks for updated X resolution
      setNumTasks(prev => Math.min(prev, Math.min(250, Math.floor(clampedX/2), Math.floor(resolution.y_resolution/2))));
    }
  };
  ...
  const updateYResolution = e => {
    ...
    if (autoAdjustPixelRatio) {
      ...
      // clamp tasks so each slice ≥2px
      setNumTasks(prev => Math.min(prev, Math.min(250, Math.floor(newX/2), Math.floor(clampedY/2))));
    } else {
      setResolution({ ...resolution, y_resolution: clampedY });
      // clamp tasks for updated Y resolution
      setNumTasks(prev => Math.min(prev, Math.min(250, Math.floor(resolution.x_resolution/2), Math.floor(clampedY/2))));
    }
  };
